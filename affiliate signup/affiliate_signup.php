...
//This requires an ajax GET call.  Here is the url field sample:
//	url: phpfileurl + '?url=' + signup + values + '&callback=?'
//The entire request needs to be in the url field on the ajax call

$postplus = '&account_status_id=3&api_key=&date_added=' . urlencode( date("m/d/Y") ) . '&signup_ip_address=' . $_SERVER['REMOTE_ADDR'];

$url = $_GET['url'] . $postplus;
//$url = 'http://mytek.la/test.php';

/*
$my_file = 'test_errors.txt';
...
if ( !$url ) {
  ...
  if ( strtolower($_SERVER['REQUEST_METHOD']) == 'POST' ) {
    ...
    curl_setopt( $ch, CURLOPT_POSTFIELDS, $postplus );
  }
